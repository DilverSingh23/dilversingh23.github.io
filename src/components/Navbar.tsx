...
export default function Navbar() {
    ...
    return (
        <nav className={`fixed w-full flex gap-6 md:gap-16 md:items-center justify-between md:justify-center max-md:px-6 z-50 ${scrollY > 10 && "backdrop-blur-md bg-black/1"}`}>
            ...
            <ul className={`${hamburgerOpen && screenWidth < 768 ? "w-full bg-black/10 py-4 backdrop-blur-md flex absolute transform -translate-x-1/2 left-1/2 mt-24" : "hidden"} md:flex items-center gap-3 flex-col md:gap-12 md:flex-row font-extralight text-white text-xl font-inter z-50`}>
